...
const NOTES_KEY = (id: string) => `study-planner-notes-${id}`;
...
export function CourseNotesPage() {
  ...
  const initialContent = id
    ? (localStorage.getItem(NOTES_KEY(id)) ?? "<p>Start writing your notes here…</p>")
    : "<p></p>";

  const doSave = useCallback(() => {
    if (!id || !editorRef.current?.editor) return;
    const html = editorRef.current.editor.getHTML();
    localStorage.setItem(NOTES_KEY(id), html);
    setSavedAt(new Date());
    setIsDirty(false);
  }, [id]);
