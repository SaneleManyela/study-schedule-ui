...
1. **Gmail App Password** — Go to [myaccount.google.com/apppasswords](https://myaccount.google.com/apppasswords), revoke `xxxxxxxxxx`, generate a new App Password, update `SMTP_PASSWORD` in `backend/.env`

2. **Resend API key** — Go to [resend.com/api-keys](https://resend.com/api-keys), revoke `xxxxxxxxx`, generate a new key, update `RESEND_API_KEY` in `backend/.env`
