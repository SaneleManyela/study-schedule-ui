...
- **Frontend:** GitHub Pages via `npm run deploy` (`gh-pages` branch, base path `/academic-truth-engine-ui/`)
- **Backend:** Docker container (`backend/Dockerfile`, multi-stage Python 3.12 slim), deployed to Google Cloud Run
- All credentials stored in GCP Secret Manager — never baked into the image

See `DEPLOYMENT.md` for full step-by-step instructions.

### Layer 6 — Cloud and computing
- **Google Cloud Run:** serverless, scales to zero on idle (`--min-instances 0`), scales out to 2 instances under load
- **Google Artifact Registry:** stores Docker images
- **Google Secret Manager:** stores all runtime secrets
- **Firestore:** fully managed, no servers to operate

### Layer 7 — CI/CD and version control
`.github/workflows/deploy.yml` runs on every push to `main`:
1. Authenticates to GCP via Workload Identity Federation (no long-lived key in CI)
2. Builds and pushes Docker image to Artifact Registry
3. Deploys to Cloud Run with secrets injected at runtime
4. Builds frontend with `VITE_API_BASE_URL` and deploys to GitHub Pages
...
- **Secrets:** never in source code or container image; loaded from GCP Secret Manager at runtime
...
Cloud Run provides automatic horizontal scaling (0 → 2 instances) based on request concurrency. Google's load balancer distributes traffic across instances. Scale-to-zero when idle means zero cost and ~3-second cold start.

### Layer 12 — Error tracking and logs
- FastAPI logs all errors via Python `logging` — captured automatically by **Cloud Logging** on GCP (queryable in the GCP console with no extra setup)
- `src/app/components/ErrorBoundary.tsx` wraps the entire React app — any unhandled render error shows a recovery UI instead of a blank white screen, and logs to the console (captured by Cloud Logging in production)
- All HTTP errors return structured JSON `{"detail": "..."}` with appropriate status codes

### Layer 13 — Availability and recovery
- `/api/health` endpoint used by Cloud Run liveness checks; unhealthy instances are replaced automatically
- Cloud Run restarts failed containers automatically
...
- `DEPLOYMENT.md`: full deployment instructions (GCP + Render.com alternative)
